...
Units are declared in TOML files (`.taba.toml`). The schema uses
**progressive disclosure**: simple cases are 3 lines, complexity is
available but never mandatory. One file = one unit.
...
| `"ephemeral"` | Auto-removed when producing bounded task terminates. | — |
| `"local-only"` | Never enters graph. Requires policy for classification > public. | — |
